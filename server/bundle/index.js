...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.0.4/node_modules/@modelcontextprotocol/sdk/dist/types.js
...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.0.4/node_modules/@modelcontextprotocol/sdk/dist/shared/protocol.js
...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.0.4/node_modules/@modelcontextprotocol/sdk/dist/server/index.js
...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.0.4/node_modules/@modelcontextprotocol/sdk/dist/server/stdio.js
import process2 from "node:process";

// node_modules/.pnpm/@modelcontextprotocol+sdk@1.0.4/node_modules/@modelcontextprotocol/sdk/dist/shared/stdio.js
...
var schemaPropertyMissingDescription = {
...
var schemaPropertyMissingType = {
  id: "schema-property-missing-type",
  run: (t) => {
    const props = t.input_schema?.properties ?? {};
    const issues = [];
    for (const [name, prop] of Object.entries(props)) {
      if (!prop || prop.type === void 0 || prop.type === null || typeof prop.type === "string" && prop.type.length === 0) {
        issues.push({
...
var nameNotSnakeCase = {
  id: "name-not-snake-case",
  run: (t) => SNAKE_CASE.test(t.name) ? [] : [{
...
// server/src/tools/grade.ts
var gradeTool = {
  name: "grade_tool_description",
  description: "Score an MCP tool definition against a deterministic rubric and return a list of ranked issues. Use when authoring or reviewing a tool's name, description, or inputSchema. Do not use to grade prompts, agent system messages, or non-MCP function specs.",
  inputSchema: {
    type: "object",
    properties: {
...
async function gradeToolHandler(args) {
  const missing = [];
  ...
  if (missing.length > 0) {
    return {
      ...
      content: [
        {
          type: "text",
          text: `Missing required argument(s): ${missing.join(", ")}. grade_tool_description requires { name, description, input_schema }.`
        }
      ]
    };
  }
...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.0.4/node_modules/@modelcontextprotocol/sdk/dist/client/index.js
...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.0.4/node_modules/@modelcontextprotocol/sdk/dist/client/stdio.js
...
async function callRemoteTool(target) {
  const start = Date.now();
  const response = await withClient(
    target,
    (client) => client.callTool({ name: target.toolName, arguments: target.arguments ?? {} })
  );
  return {
    response,
...
async function safeList(client, kind) {
  try {
    if (kind === "tools") {
      const res2 = await client.listTools();
      return { items: res2.tools ?? [] };
    }
    if (kind === "resources") {
      const res2 = await client.listResources();
      return { items: res2.resources ?? [] };
    }
    ...
    return {
      items: [],
      warning: `Could not list ${kind}: ${err.message ?? String(err)}`
    };
  }
}
...
// server/src/tools/inspect.ts
var inspectTool = {
  name: "inspect_mcp_server",
  description: "Boot a target MCP server in a child process, complete the MCP handshake, and return its declared tools, resources, and prompts as structured JSON. Use when the user is debugging or auditing an MCP server they own. Do not use to call individual tools \u2014 use call_mcp_tool for that.",
  inputSchema: {
    type: "object",
    properties: {
      command: {
...
async function inspectToolHandler(args) {
  ...
  try {
    ...
    return {
      ...
      content: [
        {
          type: "text",
          text: `inspect_mcp_server failed to connect to "${args.command}": ${err.message ?? String(err)}`
        }
      ]
    };
  }
...
// server/src/lib/tail.ts
var MIN_DURATION_MS = 100;
async function tailMcpLogs(req, spawner = realSpawner) {
  const durationMs = Math.max(MIN_DURATION_MS, Math.floor(req.durationSeconds * 1e3));
  ...
  const exit = new Promise(
    (resolveExit) => {
      child.once("exit", (code, signal2) => resolveExit({ code, signal: signal2 }));
  ...
  if (result !== "timeout") {
    ...
    const final = await Promise.race([
      exit,
      new Promise(
        (r) => setTimeout(() => r({ code: null, signal: null }), 200)
      )
    ]);
    exitCode = final.code;
    signal = final.signal;
...
// server/src/tools/tail.ts
var tailTool = {
  name: "tail_mcp_logs",
  description: "Spawn a target MCP server and capture its stderr output for a fixed duration window. Use when an MCP server appears to misbehave at startup or while serving a request \u2014 stdio servers usually surface diagnostics on stderr that the host UI swallows. Do not use to inspect the protocol traffic itself; use inspect_mcp_server for that.",
  inputSchema: {
    type: "object",
    properties: {
      command: {
...
async function tailToolHandler(args) {
  ...
  try {
    ...
    return {
      ...
      content: [
        {
          type: "text",
          text: `tail_mcp_logs failed: ${err.message ?? String(err)}`
        }
      ]
    };
  }
...
// server/src/tools/call.ts
var callTool = {
  name: "call_mcp_tool",
  description: 'Invoke a single tool on a target MCP server and return its response plus timing. Use when the user wants to exercise an MCP tool end-to-end (e.g. "run my search_tickets tool with query=billing"). Do not use to enumerate available tools \u2014 call inspect_mcp_server for that.',
  inputSchema: {
    type: "object",
    properties: {
      command: {
...
async function callToolHandler(args) {
  if (typeof args?.command !== "string" || args.command.length === 0) {
    return {
      isError: true,
  ...
  try {
    ...
    return {
      ...
      content: [
        {
          type: "text",
          text: `call_mcp_tool failed: ${err.message ?? String(err)}`
        }
      ]
    };
  }
...
function renderMarkdown(serverName, plans) {
...
async function generateSmokeTests(target) {
  const report = await inspect(target);
  const remoteTools = report.tools ?? [];
  const plans = remoteTools.map((t) => {
...
// server/src/tools/smoke.ts
var smokeTool = {
  name: "generate_smoke_tests",
  description: "Inspect a target MCP server and produce a markdown smoke-test plan: per tool, the rubric score, top rubric issue, two positive prompts that should invoke the tool, and two negative prompts that should not. Use when the user has just scaffolded an MCP server and wants a starting test plan in their repo. Do not use to actually run the prompts \u2014 that's the agent's job; this only produces the plan.",
  inputSchema: {
    type: "object",
    properties: {
      command: {
...
async function smokeToolHandler(args) {
  if (typeof args?.command !== "string" || args.command.length === 0) {
    return {
      isError: true,
      content: [{ type: "text", text: 'Missing or invalid "command".' }]
  ...
  try {
    ...
    return {
      ...
      content: [
        {
          type: "text",
          text: `generate_smoke_tests failed: ${err.message ?? String(err)}`
        }
      ]
    };
  }
...
function getRegisteredTools() {
  return [
    { definition: gradeTool, handler: gradeToolHandler },
    { definition: inspectTool, handler: inspectToolHandler },
    { definition: tailTool, handler: tailToolHandler },
    { definition: callTool, handler: callToolHandler },
    { definition: smokeTool, handler: smokeToolHandler }
  ];
}
...
var isEntrypoint = import.meta.url === `file://${process.argv[1]}`;
if (isEntrypoint) {
  main().catch((err) => {
    console.error(err);
    process.exit(1);
